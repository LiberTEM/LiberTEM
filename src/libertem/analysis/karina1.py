...
class Karina1PhaseCorrelationAnalysis(BaseAnalysis):
    ...
    def run_analysis(self, executor):
        ...
        for centers, peak_values, partition in executor.map_partitions(
                ...
                fn=self.pass_2,
                peaks=peaks):
            all_centers[partition.slice.get(nav_only=True) + (...,)] = centers
    ...
    def pass_1(self, partition):
        ...
        for tile in tiles:
            assert len(tile.data.shape) == 3, "assuming flat nav axis, FIXME!"
            data = tile.data
    ...
    def pass_2(self, partition, peaks):
        ...
        centers = np.zeros(tuple(partition.slice.shape.flatten_nav().nav) + (len(peaks), 2),
                           dtype="u2")
        peak_values = np.zeros(tuple(partition.slice.shape.flatten_nav().nav) + (len(peaks),),
                               dtype="float32")
        ...
        for tile in tiles:
            ...
            for frame_idx, frame in enumerate(data):
                ...
                for disk_idx, crop_part in enumerate(self.crop_disks_from_frame(
                    ...
                    print(abs_center, center, crop_origin)

                    result_idx = (start_of_tile + frame_idx, disk_idx)
