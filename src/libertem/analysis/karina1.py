...
class Karina1PhaseCorrelationAnalysis(BaseAnalysis):
    ...
    def run_analysis(self, executor):
        ...
        sum_result_total = zeros(dataset.shape.sig, dtype="float32")
        ...
        all_centers = zeros(tuple(dataset.raw_shape.nav) + (num_disks, 2),
                            dtype="u2")
        all_peak_values = zeros(tuple(dataset.raw_shape.nav) + (num_disks,),
                                dtype="float32")
    ...
    def pass_1(self, partition):
        ...
        sum_result = zeros(partition.meta.shape.sig, dtype="float32")

        # TODO:
        # bf_result = np.zeros(partition.meta.shape.nav, dtype="float32")

        buf = zeros(partition.meta.shape.sig, dtype="float32")
    ...
    def pass_2(self, partition, peaks):
        ...
        centers = np.zeros(tuple(partition.slice.shape.nav) + (len(peaks), 2),
                           dtype="u2")
        peak_values = zeros(tuple(partition.slice.shape.nav) + (len(peaks),),
                            dtype="float32")
        buf = zeros((2 * crop_size, 2 * crop_size), dtype="float32")
