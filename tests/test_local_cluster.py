...
@pytest.mark.skipif(detect()['cudas'] and not has_cupy(), reason="CUDA devices without functional CuPy")
def test_start_local_default(hdf5_ds_1):
    ...
    if cudas:
        assert np.all(cupy_only['backend'].data == 'cuda')
    assert np.all(numpy_only['backend'].data == 'cpu')
...
def test_start_local_cpuonly(hdf5_ds_1):
    ...
    assert np.all(udf_res['backend'].data == 'cpu')
...
def test_start_local_cupyonly(hdf5_ds_1):
    ...
    assert np.all(udf_res['backend'].data == 'cuda')
    assert np.allclose(udf_res['on_device'].data, data.sum(axis=(0, 1)))
