...
class DisplayObject:
    ...
    def __repr__(self):
        if not self._show_mem_addr:
            cls = self.__class__
            r = f"<{cls.__module__}.{cls.__name__} object>"
        else:
            r = super().__repr__()
        return r
...
class TextDisplayObject(DisplayObject):
    ...
    def _check_data(self):
        if self.data is not None and not isinstance(self.data, str):
            raise TypeError(f"{self.__class__.__name__} expects text, not {self.data!r}")
...
class JSON(DisplayObject):
    ...
    def _check_data(self):
        if self.data is not None and not isinstance(self.data, (dict, list)):
            raise TypeError(f"{self.__class__.__name__} expects JSONable dict or list, not {self.data!r}")
