...
class StatelessInputTransformer(InputTransformer):
    ...
    def __repr__(self):
        return f"StatelessInputTransformer(func={self.func!r})"
...
class CoroutineInputTransformer(InputTransformer):
    ...
    def __repr__(self):
        return f"CoroutineInputTransformer(coro={self.coro!r})"
...
def _make_help_call(target, esc, lspace, next_input=None):
    ...
    if next_input is None:
        return f'{lspace}get_ipython().run_line_magic({t_magic_name!r}, {t_magic_arg_s!r})'
    else:
...
def _tr_system(line_info):
    ...
    return f'{line_info.pre}get_ipython().system({cmd!r})'

def _tr_system2(line_info):
    "Translate lines escaped with: !!"
    cmd = line_info.line.lstrip()[2:]
    return f'{line_info.pre}get_ipython().getoutput({cmd!r})'
...
assign_system_re = re.compile(fr'{_assign_pat}!\s*(?P<cmd>.*)', re.VERBOSE)
...
assign_magic_re = re.compile(fr'{_assign_pat}%\s*(?P<cmd>.*)', re.VERBOSE)
