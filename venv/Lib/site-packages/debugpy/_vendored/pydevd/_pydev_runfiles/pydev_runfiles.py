    ...
    for opt, value in optlist:
        if opt in ("-v", "--verbosity"):
            ...
            if split_jobs not in ('module', 'tests'):
                raise AssertionError(f'Expected split to be either "module" or "tests". Was :{split_jobs}')
            ...
            if os.path.exists(config_file):
                ...
                sys.stderr.write(f'Could not find config file: {config_file}\n')
...
class PydevTestRunner:
        ...
        for n in xrange(len(self.files_or_dirs)):
            ...
            if os.path.isdir(dir_name):
                ...
                if not os.path.exists(dir_name):
                    block_line = '*' * 120
                    sys.stderr.write(f'\n{block_line}\n* PyDev test runner error: {dir_name} does not exist.\n{block_line}\n')
                    return
    ...
    def find_import_files(self):
        ...
        if self.configuration.exclude_files or self.configuration.include_files:
            ...
            for f in pyfiles:
                ...
                if not add:
                    if self.verbosity > 3:
                        sys.stdout.write(f'Skipped file: {f} (did not match any include_files pattern: {self.configuration.include_files})\n')

                elif self.configuration.exclude_files:
                    for pat in self.configuration.exclude_files:
                        if fnmatch.fnmatchcase(basename, pat):
                            if self.verbosity > 3:
                                sys.stdout.write(f'Skipped file: {f} (matched exclude_files pattern: {pat})\n')

                            elif self.verbosity > 2:
                                sys.stdout.write(f'Skipped file: {f}\n')
                ...
                if add:
                    if self.verbosity > 3:
                        sys.stdout.write(f'Adding file: {f} for test discovery.\n')
                    ret.append(f)
        ...
        try:
            ...
            if print_exception:
                ...
                try:
                    ...
                    sys.stderr.write(f'ERROR: Module: {modname} could not be imported (file: {pyfile}).\n')
                finally:
        ...
        if not internal_call:
            ...
            if self.verbosity > 1:
                if self.configuration.include_tests:
                    sys.stdout.write(f'Tests to include: {self.configuration.include_tests}\n')

                if self.tests:
                    sys.stdout.write(f'Tests to run: {self.tests}\n')

                if self.configuration.exclude_tests:
                    sys.stdout.write(f'Tests to exclude: {self.configuration.exclude_tests}\n')
                ...
                if self.configuration.exclude_tests:
                    for pat in self.configuration.exclude_tests:
                        if fnmatch.fnmatchcase(testMethodName, pat):
                            if self.verbosity > 3:
                                sys.stdout.write(f'Skipped test: {testMethodName} (matched exclude_tests pattern: {pat})\n')

                            elif self.verbosity > 2:
                                sys.stdout.write(f'Skipped test: {testMethodName}\n')
                ...
                if add:
                    if self.__match_tests(self.tests, test_obj, testMethodName):
                        ...
                        if include:
                            ...
                            if self.verbosity > 3:
                                sys.stdout.write('Skipped test: {} (did not match any include_tests pattern {})\n'.format(
    ...
    def run_tests(self, handle_coverage=True):
        ...
        total_time = f'Finished in: {time.time() - start_time:.2f} secs.'
