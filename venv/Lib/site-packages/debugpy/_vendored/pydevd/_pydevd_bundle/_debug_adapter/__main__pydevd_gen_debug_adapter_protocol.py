    ...
    if enum_type and len(enum_type) == 1 and next(iter(enum_type)) in ("request", "response", "event"):
        ...
        if command:
            ...
            if enum and len(enum) == 1:
                class_to_generate['register_request'] = f'@register_{msg_type}({enum[0]!r})\n'
        ...
        if enum and len(enum) == 1:
            ...
            if prop['type'].__class__ == Ref:
                ...
                if ref_data.get('is_enum', False):
                    init_body.append(f'    if {prop_name} is not None:')
                    init_body.append(f'        assert {prop_name} in {str(ref)}.VALID_VALUES')
