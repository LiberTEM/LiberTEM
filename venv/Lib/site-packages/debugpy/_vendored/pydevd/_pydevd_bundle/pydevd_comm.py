        ...
        try:
            ...
            while True:
                ...
                if DebugInfoHolder.DEBUG_RECORD_SOCKET_READS:
                    pydev_log.critical(f'debugger: received >>{line}<<\n')

                args = line.split('\t', 2)
                try:
                    cmd_id = int(args[0])
                    pydev_log.debug('Received command: {} {}\n'.format(ID_TO_MEANING.get(str(cmd_id), '???'), line))
    ...
    try:
        ...
        if not modules_to_reload:
            if filename and module_name:
                _send_io_message(dbg, f'code reload: Unable to find module {module_name} to reload for path: {filename}\n')
            elif filename:
                _send_io_message(dbg, f'code reload: Unable to find module to reload for path: {filename}\n')
            elif module_name:
                _send_io_message(dbg, f'code reload: Unable to find module to reload: {module_name}\n')
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, f"Error resolving variables {get_exception_traceback_str()}")
            dbg.writer.add_command(cmd)
...
def internal_change_variable(dbg, seq, thread_id, frame_id, scope, attr, value):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(seq, f"Error changing variable attr:{attr} expression:{value} traceback:{get_exception_traceback_str()}")
        dbg.writer.add_command(cmd)
...
def internal_change_variable_json(py_db, request):
    ...
    if variable is None:
        _write_variable_response(
            py_db, request, value='', success=False, message=f'Unable to find variable container to change: {variables_reference}.')
        return
    ...
    if child_var is None:
        _write_variable_response(
            py_db, request, value='', success=False, message=f'Unable to change: {arguments.name}.')
        return
...
def internal_get_frame(dbg, seq, thread_id, frame_id):
    ...
    try:
        ...
        if frame is not None:
            ...
            cmd = dbg.cmd_factory.make_error_message(seq, f"Frame not found: {frame_id} from thread: {thread_id}")
            dbg.writer.add_command(cmd)
    except:
        cmd = dbg.cmd_factory.make_error_message(seq, f"Error resolving frame: {frame_id} from thread: {thread_id}")
        dbg.writer.add_command(cmd)
        ...
        for variant in variants:
            if variant.children_variants:
                for child_variant in variant.children_variants:
                    ...
                    xml += '<variant name="{}" isVisited="{}" line="{}" offset="{}" childOffset="{}" callOrder="{}"/>'.format(
                ...
                xml += '<variant name="{}" isVisited="{}" line="{}" offset="{}" childOffset="-1" callOrder="{}"/>'.format(
                    quote(variant.name),
                ...
                if variant.children_variants:
                    for child_variant in variant.children_variants:
                        ...
                        if child_variant.call_order > 1:
                            targets.append(StepInTarget(id=target_id, label=f'{child_variant.name} (call {child_variant.call_order})',))
                        else:
                    ...
                    if variant.call_order > 1:
                        targets.append(StepInTarget(id=target_id, label=f'{variant.name} (call {variant.call_order})',))
                    else:
...
def internal_evaluate_expression_json(py_db, request, thread_id):
    ...
    with ctx:
        ...
        if frame_tracker is None:
            ...
            _evaluate_response(py_db, request, result='', error_message=f'Thread id: {thread_id} is not current thread id.')
            return
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    if is_error:
        _set_expression_response(py_db, request, result, error_message=f'Error executing: {exec_code}')
        return
    ...
    if frame_tracker is None:
        # This is not really expected.
        _set_expression_response(py_db, request, result, error_message=f'Thread id: {thread_id} is not current thread id.')
        return
    ...
    try:
        ...
        try:
            ...
            if frame is not None:
                ...
                cmd = dbg.cmd_factory.make_error_message(seq, f"internal_get_completions: Frame not found: {frame_id} from thread: {thread_id}")
                dbg.writer.add_command(cmd)
