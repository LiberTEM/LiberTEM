...
class ConsoleMessage:
    ...
    def to_xml(self):
        ...
        for message_type, message in self.console_messages:
            xml += f'<{message_type} message="{makeValid(message)}"></{message_type}>'
    ...
    def push(self, line, frame, buffer_output=True):
        ...
        try:
            try:
                ...
                if buffer_output:
                    err.buflist.append(f"Internal Error: {exc}")
                else:
...
def get_interactive_console(thread_id, frame_id, frame, console_message):
    ...
    if console_stacktrace:
        ...
        context_message = f'File "{current_context[0]}", line {current_context[1]}, in {current_context[2]}'
        console_message.add_console_message(CONSOLE_OUTPUT, f"[Current context]: {context_message}")
    return InteractiveConsoleCache.interactive_console_instance
