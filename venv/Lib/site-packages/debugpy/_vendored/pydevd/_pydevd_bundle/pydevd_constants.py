...
def as_float_in_env(env_key, default):
    ...
    try:
        ...
        raise RuntimeError(
            'Error: expected the env variable: {} to be set to a float value. Found: {}'.format(
                env_key, value))
...
if USE_CYTHON_FLAG is not None:
    ...
    if USE_CYTHON_FLAG not in ENV_TRUE_LOWER_VALUES and USE_CYTHON_FLAG not in ENV_FALSE_LOWER_VALUES:
        raise RuntimeError('Unexpected value for PYDEVD_USE_CYTHON: {} (enable with one of: {}, disable with one of: {})'.format(
...
def protect_libraries_from_patching():
    ...
    patched_modules = {k: v for k, v in sys.modules.items()
                            if k in patched}
...
def set_protocol(protocol):
    ...
    assert protocol in expected, 'Protocol ({}) should be one of: {}'.format(
        protocol, expected)
