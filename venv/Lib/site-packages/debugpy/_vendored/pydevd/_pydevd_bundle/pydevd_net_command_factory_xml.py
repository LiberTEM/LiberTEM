...
class NetCommandFactory:
    ...
    def _thread_to_xml(self, thread):
        ...
        cmdText = f'<thread name="{quote(name)}" id="{get_thread_id(thread)}" />'
        return cmdText
    ...
    def make_custom_frame_created_message(self, frame_id, frame_description):
        ...
        return NetCommand(CMD_THREAD_CREATE, 0, f'<xml><thread name="{frame_description}" id="{frame_id}"/></xml>')
    ...
    def make_io_message(self, msg, ctx):
        ...
        try:
            ...
            return NetCommand(str(CMD_WRITE_TO_CONSOLE), 0, f'<xml><io s="{msg}" ctx="{ctx}"/></xml>')
        except:
            return self.make_error_message(0, get_exception_traceback_str())
    ...
    def make_thread_run_message(self, thread_id, reason):
        try:
            return NetCommand(CMD_THREAD_RUN, 0, f"{thread_id}\t{reason}")
        except:
            return self.make_error_message(0, get_exception_traceback_str())
                    ...
                    while frame is not None:
                        if frame.f_code.co_name == 'do_wait_suspend' and frame.f_code.co_filename.endswith('pydevd.py'):
                            ...
                            if arg is not None:
                                ...
                                cmd_text.append(f'exc_desc="{exc_desc}" ')
                                cmd_text.append('>')
