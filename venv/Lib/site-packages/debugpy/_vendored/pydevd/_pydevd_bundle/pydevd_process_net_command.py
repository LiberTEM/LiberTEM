    ...
    def cmd_set_break(self, py_db, cmd_id, seq, text):
        ...
        if error_code:
            ...
            if error_code == self.api.ADD_BREAKPOINT_FILE_NOT_FOUND:
                pydev_log.critical(f'pydev debugger: warning: Trying to add breakpoint to file that does not exist: {translated_filename} (will have no effect).')

            elif error_code == self.api.ADD_BREAKPOINT_FILE_EXCLUDED_BY_FILTERS:
                pydev_log.critical(f'pydev debugger: warning: Trying to add breakpoint to file that is excluded by filters: {translated_filename} (will have no effect).')

            else:
                # Shouldn't get here.
                pydev_log.critical(f'pydev debugger: warning: Breakpoint not validated (reason unknown -- please report as error): {translated_filename}.')
    ...
    def cmd_run_custom_operation(self, py_db, cmd_id, seq, text):
        ...
        if text != "":
            try:
                ...
                sys.stderr.write(f'Custom operation now needs a || separator. Found: {text}\n')
                raise
