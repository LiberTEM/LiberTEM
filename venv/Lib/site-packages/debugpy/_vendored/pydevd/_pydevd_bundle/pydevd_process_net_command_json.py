    ...
    if not isinstance(rules, list):
        ...
        for rule in rules:
            ...
            if path is None and module is None:
                on_error(f'Invalid "rule" (expected dict with "path" or "module"). Found: {rule}')
                continue
...
class PyDevJsonCommandProcessor:
    ...
    def process_net_command_json(self, py_db, json_contents, send_response=True):
        ...
        try:
            ...
            if on_request is None:
                print(f'Unhandled: {method_name} not available in PyDevJsonCommandProcessor.\n')
                return

            if DEBUG:
                print(f'Handled in pydevd: {method_name} (in PyDevJsonCommandProcessor).\n')
    ...
    def _set_debug_options(self, py_db, args, start_reason):
        ...
        if isinstance(variable_presentation, dict):
            ...
            def get_variable_presentation(setting, default):
                ...
                if value not in ('group', 'inline', 'hide'):
                    pydev_log.info(
                        'The value set for "{}" ({}) in the variablePresentation is not valid. Valid values are: "group", "inline", "hide"'.format(
                            setting, value))
                    value = default
        ...
        watch_dirs = {pydevd_file_utils.get_path_with_real_case(w) for w in watch_dirs}
        ...
        if target_id is not None:
            ...
            if variant is not None:
                ...
                variables_response = pydevd_base_schema.build_response(
                    ...
                    kwargs={
                        'success': False,
                        'message': 'Unable to find step into target {}. Available targets: {}'.format(
