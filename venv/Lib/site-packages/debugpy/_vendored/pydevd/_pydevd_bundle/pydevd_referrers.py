...
def print_referrers(obj, stream=None):
    ...
    for node in xml.childNodes:
        ...
        if node.localName == 'for':
            ...
            sys.stderr.write(f'Unhandled node: {node}\n')
...
def get_referrer_info(searched_obj):
    ...
    try:
        try:
            ...
            if DEBUG:
                sys.stderr.write(f'Found {len(referrers)} referrers.\n')
            ...
            ret.append('<for>\n')
            if DEBUG:
                sys.stderr.write(f'Searching Referrers of obj with id="{obj_id}"\n')

            ret.append(pydevd_xml.var_to_xml(
                searched_obj,
                f'Referrers of obj with id="{obj_id}"'))
            ret.append('</for>\n')
            ...
            for r in referrers:
                ...
                if r_type == frame_type:
                    if DEBUG:
                        sys.stderr.write(f'Found frame referrer: {r!r}\n')
                    ...
                    if DEBUG:
                        sys.stderr.write(f'Found dict referrer: {r!r}\n')
                    ...
                    for key, val in r.items():
                        if val is searched_obj:
                            found_as = key
                            if DEBUG:
                                sys.stderr.write(f'    Found as {found_as!r} in dict\n')
                            break
                    ...
                    if DEBUG:
                        sys.stderr.write(f'Found tuple referrer: {r!r}\n')

                    for i, x in enumerate(r):
                        if x is searched_obj:
                            found_as = f'{r_type.__name__}[{i}]'
                            if DEBUG:
                                sys.stderr.write(f'    Found as {found_as} in tuple: \n')
                            break
