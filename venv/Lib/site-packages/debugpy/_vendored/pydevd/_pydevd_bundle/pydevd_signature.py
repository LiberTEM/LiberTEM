    ...
    if class_name == 'dict':
        ...
        if len(value) > 0 and recursive:
            for (k, v) in dict_iter_items(value):
                class_name += '[{}, {}]'.format(get_type_of_value(k, recursive=recursive),
...
def create_signature_message(signature):
    ...
    cmdTextList.append(f'<call_signature file="{pydevd_xml.make_valid_xml_value(signature.file)}" name="{pydevd_xml.make_valid_xml_value(signature.name)}">')

    for arg in signature.args:
        cmdTextList.append(f'<arg name="{pydevd_xml.make_valid_xml_value(arg[0])}" type="{pydevd_xml.make_valid_xml_value(arg[1])}"></arg>')
