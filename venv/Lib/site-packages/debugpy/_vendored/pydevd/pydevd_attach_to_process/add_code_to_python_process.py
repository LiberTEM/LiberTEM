...
def run_python_code_windows(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    with _acquire_mutex(f'_pydevd_pid_attach_mutex_{pid}', 10):
        print(f'--- Connecting to {bits} bits target (current process is: {64 if is_python_64bit() else 32}) ---')
        ...
        with _win_write_to_shared_named_memory(python_code, pid):
            ...
            print(f'\n--- Injecting attach dll: {name} into pid: {pid} ---')
            ...
            with _create_win_event(f'_pydevd_pid_event_{pid}') as event:
                print(f'\n--- Injecting run code dll: {name} into pid: {pid} ---')
                args = [target_executable, str(pid), target_dll]
                subprocess.check_call(args)
...
def run_python_code_linux(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    print(f'Attaching with arch: {arch}')
...
#         '--batch-silent',
    ...
    cmd.extend([
        ...
        "--eval-command='call (int)DoAttach({}, \"{}\", {})'".format(
...
def run_python_code_mac(pid, python_code, connect_debugger_tracing=False, show_debug_info=0):
    ...
    cmd.extend([
        ...
        "-o 'load_lib_and_attach \"{}\" {} \"{}\" {}'".format(target_dll,
            is_debug, python_code, show_debug_info),
    ])
