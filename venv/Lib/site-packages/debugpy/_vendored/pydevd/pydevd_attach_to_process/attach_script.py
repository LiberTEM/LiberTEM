    ...
    if len(possible_thread_ids) > 0:
        ...
        if len(possible_thread_ids) == 1:
            ...
            return possible_thread_ids[0], \
                'Multiple thread ids found ({}). Choosing main thread id randomly ({}).'.format(
...
def fix_main_thread_id(on_warn=lambda msg:None, on_exception=lambda msg:None, on_critical=lambda msg:None):
    ...
    try:
        ...
        if critical_warning:
            on_critical(f'Issue found when debugger was trying to work around https://bugs.python.org/issue37416:\n{critical_warning}')
    except:
