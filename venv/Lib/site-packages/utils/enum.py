...
class _enum:
    ...
    def __iter__(self):
        yield from self.__enum_items.items()

    def __repr__(self):
        return f"<{self.__class__.__name__}: {self.__enum_items.values()}>"
enum = _enum()
...
class _EnumMeta(type):
    def __new__(cls, name, bases, attr_dict):
        ...
        new_enum = super().__new__(cls, name, bases, {})

        enum_items = {}

        for attr_name, attr_value in attr_dict.items():
            if attr_name.startswith('__'):
                super().__setattr__(new_enum, attr_name, attr_value)
                continue
            ...
            super().__setattr__(new_enum, attr_name, enum_item)

        if getattr(options, "frozen", True):
            super().__setattr__(new_enum, '__frozen__', True)
        else:
            super().__setattr__(new_enum, '__frozen__', False)

        if getattr(options, "strict", False):
            super().__setattr__(new_enum, '__strict__', True)
        else:
            super().__setattr__(new_enum, '__strict__', False)

        super().__setattr__(new_enum, '__enum_item_map__', enum_items)
    ...
    def get_name_value_map(cls):
        ...
        return {e[i].name: e[i].value for i in e}
